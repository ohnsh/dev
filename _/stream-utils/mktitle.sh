title() {
  ...
  ffmpeg -f lavfi -i "color=color=#1a1a2e:s=1920x1080:d=5:r=30" \
    -vf "drawtext=fontfile=/path/to/font.ttf:text='STREAM STARTING SOON':fontcolor=white:fontsize=48:x=(w-text_w)/2:y=(h-text_h)/2" \
...
title_gradient() {
  ffmpeg -f lavfi -i "nullsrc=s=1920x1080:d=5:r=30" \
    -vf "geq=r='X/W*255':g='0':b='Y/H*255', gblur=sigma=100,
     drawtext=fontfile=/path/to/font.ttf:text='LIVE STREAMING':fontcolor=white:fontsize=64:x=(w-text_w)/2:y=(h-text_h)/2" \
    -c:v libx264 -pix_fmt yuv420p -g 60 title_card.mp4
}

title_retro() {
  ffmpeg -f lavfi -i "smptebars=s=1920x1080:d=5:r=30" \
    -vf "drawtext=fontfile=/path/to/font.ttf:text='INITIALIZING FEED...':fontcolor=white:box=1:boxcolor=black@0.8:boxborderw=20:fontsize=48:x=(w-text_w)/2:y=(h-text_h)/2" \
    -c:v libx264 -pix_fmt yuv420p -g 60 title_card.mp4
}
...
title_bgimage() {
  ...
  ffmpeg -loop 1 -i background.png -t 5 \
    -vf "drawtext=fontfile=/path/to/font.ttf:text='STREAM STARTING SOON':fontcolor=white:fontsize=48:x=(w-text_w)/2:y=(h-text_h)/2,
     drawtext=fontfile=/path/to/font.ttf:text='%{localtime\:%X}':fontcolor=yellow:fontsize=32:x=(w-text_w)/2:y=(h-text_h)/2+60" \
